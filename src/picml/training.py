...
def default_lr_scheduler(epoch: int, initial_learning_rate: float = 1e-3, decay: float = 0.975) -> float:
    """
    Simple exponential decay learning-rate schedule.

    lr(epoch) = initial_learning_rate * decay**epoch, with no decay for epoch 0–1.
    """
    if epoch <= 1:
...
def train_case_A_model2(
    new_X_train_scaled: np.ndarray,
    new_y_train_scaled: np.ndarray,
    new_X_test_scaled: np.ndarray,
    ...
    model_sc = create_model_A_2(input_shape=new_X_train_scaled_cnn.shape[1:])
...
def train_case_B_model1(
    ...
    callbacks = []
    if use_lr_schedule:
        lr_scheduler_callback = LearningRateScheduler(default_lr_scheduler)
